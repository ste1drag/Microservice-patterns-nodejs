...
const establish = async () => {
    ...
    connection.on("error", (err) => console.error("RabbitMQ greska pri konekciji:", err.message));
    connection.on("close", () => {
        if (closing) return;
        console.warn(`RabbitMQ konekcija zatvorena, ponovno pokretanje za ${RECONNECT_DELAY_MS}ms`);
        channel = null;
        connection = null;
        setTimeout(() => {
            establish().catch((err) => console.error("RabbitMQ rekonekcije neuspesna:", err.message));
        }, RECONNECT_DELAY_MS);
    });

    if (onReady) await onReady(channel);

    console.log("RabbitMQ konektovan i topologija podesena");
};
